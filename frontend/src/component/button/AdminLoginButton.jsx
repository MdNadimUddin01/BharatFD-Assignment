...
const AdminLoginButton = () => {
  ...
  const { isAuthenticated } = useAuth(); // If you're using auth context
    ...
    <div className="fixed top-4 right-4 z-50">
      {isAuthenticated ? (
        <button
          ...
          className="group relative inline-flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors duration-200 shadow-sm"
        >
          <svg 
            className="w-5 h-5" 
            fill="none" 
